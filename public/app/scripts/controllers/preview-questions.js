...
.controller('PreviewQuestionsCtrl', ['$scope', '$state', '$rootScope', '$stateParams', '$sce', 'APP_PATH', 'Notifications','Line', 'Modal', function($scope, $state, $rootScope, $stateParams, $sce, APP_PATH, Notifications, Line, Modal) {

	// titre de l'action
	if ($stateParams.id.split("_")[0] == "preview") {
		var numQuestion = parseInt($stateParams.id.split("_")[1]);
	} else{
		var numQuestion = parseInt($stateParams.id);
	};
  ...
  $scope.connect2 = {id: null, x2: null, y2: null};
	...
	$scope.question = angular.copy(_.find($rootScope.tmpQuiz.questions, function(q){
		if (q.id == $stateParams.id) {
			if (q.type === 'qcm') {
				for (var i = q.answers.length - 1; i >= 0; i--) {
					q.answers[i].solution = false;
				};
			};
			if (q.type === 'ass') {
				for (var i = q.answers.length - 1; i >= 0; i--) {
					q.answers[i].leftProposition.solutions = [];
					q.answers[i].rightProposition.solutions = [];
				};
			};
			if (q.type === 'tat') {
				//on récupère les leurres
				_.each(q.leurres, function(leurre){
	...
	$scope.quit = function(){
	 	$state.go('quizs.update_questions', {id: $scope.question.id});
	}
