...
describe 'QuizTest' do
  ...
  it 'Récupère un quiz par son id' do
    found_quiz = Quiz.new(id: @datas_bdd[:quizs][0].id)
  ...
  it 'Retourne tous les quizs partagés sauf les siens' do
    shared_quizs = Quiz.new(opt_shared: true, user_id: MOCKED_DATA[:uid])
    shared_quizs = shared_quizs.find_all
    expect(shared_quizs.count).to eq(3)
  end
  ...
  it "Met à jour le nom ainsi que les paramètres d'un quiz" do
    ...
    DB.transaction do
      updated_quiz.update
      bdd_my_quiz_updated = Quizs[id: @datas_bdd[:quizs][1].id]
  ...
  it 'Supprime un quiz par rapport à son id' do
    deleted_quiz = Quiz.new(id: @datas_bdd[:quizs][0].id)
    DB.transaction do
      deleted_quiz.delete
      bdd_my_quiz_deleted = Quizs[id: @datas_bdd[:quizs][0].id]
  ...
  it 'duplique un quiz avec ses questions, suggestions et solutions dans la bdd' do
    DB.transaction do
      duplicated_quiz = Quiz.new(id: @datas_bdd[:quizs][0].id, user_id: MOCKED_DATA[:uid])
      duplicated_quiz = duplicated_quiz.duplicate
      duplicated_quiz_bdd = Quizs[id: duplicated_quiz.id]
      expect(duplicated_quiz_bdd).to_not be_nil
      expect(duplicated_quiz_bdd.title).to eq('Quiz numéro 0')
      expect(duplicated_quiz_bdd.opt_show_score).to eq('after_each')
      expect(duplicated_quiz_bdd.opt_show_correct).to eq('after_each')
      expect(duplicated_quiz_bdd.opt_can_redo).to eq(true)
      expect(duplicated_quiz_bdd.opt_can_rewind).to eq(true)
      expect(duplicated_quiz_bdd.opt_shared).to eq(false)
      expect(duplicated_quiz_bdd.opt_rand_question_order).to eq(false)

      duplicated_questions_bdd = Questions.where(quiz_id: duplicated_quiz.id)
