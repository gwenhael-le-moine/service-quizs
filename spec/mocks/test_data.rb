...
def generate_test_data(types={})
  ...
  if types[:quizs] && types[:questions] 
    ...
    questions.push(Questions.create(get_test_question({
      id: 0,
      quiz_id: quizs[0].id,
      type: 'QCM',
      order: '0',
  ...
  if types[:quizs] && types[:questions] && types[:suggestions]
    ...
    suggestions.push(Suggestions.create(get_test_suggestion({
      id: 0,
